...
describe Narabikae::Position do
  ...
  describe '#find_position_after' do
    ...
    describe 'option has scope' do
      ...
      let(:target) { Task.new(position: 'a0', user_id: 1) }
    ...
    context 'when target model mismatches' do
      let(:target) { Course.new }
    ...
    context 'when target scope mismatches' do
      let(:position) {
        described_class.new(
          Task.new(user_id: 1),
          Narabikae::Option.new(
      ...
      let(:target) { Task.new(position: 'a0', user_id: 2) }

      it 'raises a descriptive scope mismatch error' do
        expect { subject }.to raise_error(Narabikae::Error) { |error|
          expect(error.message).to include('scope', 'user_id')
        }
      end
    end
  end
  ...
  describe '#find_position_before' do
    ...
    context 'when option has scope' do
      ...
      let(:target) { Task.new(position: 'a1', user_id: 1) }
  ...
  describe '#find_position_at' do
    subject { position.find_position_at(target) }
    ...
    context 'when target is an index' do
      let(:target) { 0 }
      let(:expected) { FractionalIndexer.generate_keys(count: target + 1).last }

      it { is_expected.to eq(expected) }
    end

    context 'when target is same model and same scope' do
      let(:target) { Task.new(position: 'a2', user_id: 1) }

      it { is_expected.to eq('a2') }
    end

    context 'when target is same model but different scope' do
      let(:target) { Task.new(position: 'a2', user_id: 2) }
    ...
    context 'when target is different model' do
      let(:target) { Course.new }
    ...
    context 'when target is missing the position field' do
      let(:target) { Task.new(position: 'a2', user_id: 1) }

      before do
        allow(target).to receive(:respond_to?).and_call_original
        allow(target).to receive(:respond_to?).with(:position).and_return(false)
      end

      it { expect { subject }.to raise_error(Narabikae::Error, /field/i) }
    end

    context 'when target is an STI subclass with same scope' do
      before do
        stub_const('SpecialTask', Class.new(Task))
      end

      let(:target) { SpecialTask.new(position: 'a3', user_id: 1) }
